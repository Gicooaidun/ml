...
def setup_parser():
    ...
    parser.add_argument("--tilenames", type=str, default="/scratch2/biomass_estimation/code/ml/inference/data_manipulation/tile_names_inference.txt")
...
if __name__ == "__main__":
    ...
    # print(path_shp, tilenames, path_s2, path_icesat, norm_path, model_path)
        ...
        for t in range(len(s2_bands)):
            ...
            for i in range(100, 400, 100):
                for j in range(100, 400, 100):
                    ...
                    data = []
                    s2_temp = s2_bands[t][i-100:i+101,j-100:j+101,:]
                    data.extend([s2_temp])                
                    ...
                    icesat_temp_bm = upsampled_icesat[t]['bm'][i-100:i+101,j-100:j+101, np.newaxis]
                    icesat_temp_std = upsampled_icesat[t]['std'][i-100:i+101,j-100:j+101, np.newaxis]
                    ...
                    with torch.no_grad():
                        if torch.cuda.is_available():
                            data = data.cuda()
                        outputs[t][i-100:i+101,j-100:j+101, 0] = model(data.unsqueeze(0))[0].detach().cpu().numpy().squeeze()
                        outputs[t][i-100:i+101,j-100:j+101, 1] = model(data.unsqueeze(0))[1].detach().cpu().numpy().squeeze()
            print('>>> Inference done for tile ', t+1)

        print("saving outputs for tile ", s2_prod)
        # Save the outputs as a .npy file
        np.save(f'/scratch2/biomass_estimation/code/ml/inference/done_inference_tiles/debugging_bigpatch_{s2_prod}.npy', outputs)
        del outputs
        del s2_bands
        del icesat_norm
