...
def setup_parser():
    ...
    parser.add_argument("--model_path", type=str, default="/scratch2/biomass_estimation/code/ml/saved_model2.pth")
...
if __name__ == "__main__":
    ...
    for t in range(len(s2_bands)):
        ...
        outputs.append(np.zeros((icesat['bm'].shape[0], icesat['bm'].shape[1])))
        ...
        for i in range(7, icesat['bm'].shape[0], 15):
            for j in range(7, icesat['bm'].shape[1], 15):
                ...
                outputs[t][i-7:i+8, j-7:j+8] = model(data.unsqueeze(0)).detach().numpy().squeeze()
